const stickyClassName = 'table-sticky';
const leftFixedClass = '.el-table__fixed';
const rightFixedClass = '.el-table__fixed-right';
const canScrollValueList = ['auto', 'scroll'];

let scrollEl = null;
let scrollParentEl = null; // 监听滚动的父元素
let offset = 0; // 距离浏览器窗口顶部的值
...
const setFixedHeader = (className, isSticky, isRight) => {
...
export default {
  bind: (el, binding) => {
    setTimeout(() => {
      ...
      scrollEl = el;
      scrollParentEl = value.scrollParent ? document.querySelector(value.scrollParent)
        : getScrollEl(el);
