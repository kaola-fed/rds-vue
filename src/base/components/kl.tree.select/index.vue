...
<el-dropdown trigger="click"
    ...
    <div class="kl-tree-select" v-if="!canEdit">
        <el-input v-if="showAllSelected"
            v-model="filterName"
            class="kl-tree-select-filterName"
            placeholder="输入关键字搜索" />
        <div class="kl-tree-select-nameCtn">
            <el-tag :key="index"
                    v-for="(item, index) in checkedNamesShow"
            ...
            <el-button v-if="checkedNames.length > showSelectedCount"
                class="kl-tree-select-toggleName"
    ...
    <el-dropdown-menu slot="dropdown" class="kl-tree-dropdown-menu">
        <el-container>
            <el-aside v-if="showButtons && multiple" class="kl-tree-dropdown-aside" width="80px">
            ...
            <el-main>
                ...
                <el-tree
                    ...
                    <span class="kl-custom-tree-node" slot-scope="{ node, data }">
                        <span>{{ node.label }}</span>
                        <span v-if="!multiple">
                            <el-button type="text"
                                       size="mini"
                                       @click="singleSelect($event, node, data)">
                                选择
                            </el-button>
                        </span>
                    </span>
